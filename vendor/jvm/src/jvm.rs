...
type NativeRoots = dyn Fn() -> Option<Vec<usize>> + Send + Sync;

struct JvmInner {
    native_roots: RwLock<Option<Box<NativeRoots>>>,
...
impl Jvm {
    ...
    /// Include references held outside JVM frames (for example, AOT guest state).
    /// Candidates are matched only against live object identities. Returning None
    /// means a complete snapshot was unavailable; collection is skipped safely.
    /// The callback must not re-enter the JVM or change guest state.
    pub fn set_native_root_provider(&self, provider: impl Fn() -> Option<Vec<usize>> + Send + Sync + 'static) {
        *self.inner.native_roots.write() = Some(Box::new(provider));
    }

    pub fn collect_garbage(&self) -> Result<usize> {
        tracing::trace!("Collecting garbage");

        let native_roots = match self.inner.native_roots.read().as_ref() {
            Some(provider) => match provider() {
                Some(roots) => roots.into_iter().collect::<HashSet<_>>(),
                None => {
                    tracing::warn!("Skipping GC: native root snapshot unavailable");
                    return Ok(0);
                }
            },
            None => HashSet::new(),
