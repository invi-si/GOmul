...
#[tokio::test]
async fn null_byte_constructor_throws_instead_of_panicking() -> Result<()> {
    let jvm = test_jvm().await?;
    let null: ClassInstanceRef<Array<i8>> = None.into();
    let result = jvm.new_class("java/lang/String", "([B)V", (null,)).await;
    let Err(JavaError::JavaException(exception)) = result else { panic!("Expected NullPointerException"); };
    assert!(jvm.is_instance(&*exception, "java/lang/NullPointerException"));
    Ok(())
}
