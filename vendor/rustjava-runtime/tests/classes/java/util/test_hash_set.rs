...
use rustjava_runtime::classes::java::{lang::Object, util::HashMapEntry};
...
async fn test_hash_set_add_duplicate_contains_remove_null_and_clear() -> Result<()> {
    ...
    let added: bool = jvm
        .invoke_virtual(&hash_set, &hash_set.class_definition().name(), "add", "(Ljava/lang/Object;)Z", (value,))
        .await?;
    assert!(added);
    let added: bool = jvm
        .invoke_virtual(
    ...
    let size: i32 = jvm
        .invoke_virtual(&hash_set, &hash_set.class_definition().name(), "size", "()I", ())
        .await?;
    assert_eq!(size, 2);
    let is_empty: bool = jvm
        .invoke_virtual(&hash_set, &hash_set.class_definition().name(), "isEmpty", "()Z", ())
        .await?;
    assert!(!is_empty);
    ...
    let _: () = jvm
        .invoke_virtual(&hash_set, &hash_set.class_definition().name(), "clear", "()V", ())
        .await?;
    let size: i32 = jvm
        .invoke_virtual(&hash_set, &hash_set.class_definition().name(), "size", "()I", ())
        .await?;
...
async fn test_hash_set_to_array_and_iterator_snapshot_exhaustion_remove() -> Result<()> {
    ...
    let _: bool = jvm
        .invoke_virtual(&hash_set, &hash_set.class_definition().name(), "add", "(Ljava/lang/Object;)Z", (first,))
        .await?;
    let _: bool = jvm
        .invoke_virtual(&hash_set, &hash_set.class_definition().name(), "add", "(Ljava/lang/Object;)Z", (second,))
        .await?;

    let array: ClassInstanceRef<Array<Object>> = jvm
        .invoke_virtual(&hash_set, &hash_set.class_definition().name(), "toArray", "()[Ljava/lang/Object;", ())
        .await?;
    ...
    let iterator: ClassInstanceRef<Object> = jvm
        .invoke_virtual(&hash_set, &hash_set.class_definition().name(), "iterator", "()Ljava/util/Iterator;", ())
        .await?;
    assert!(jvm.is_instance(&**iterator, "java/util/Iterator"));

    let _: bool = jvm
        .invoke_virtual(&hash_set, &hash_set.class_definition().name(), "add", "(Ljava/lang/Object;)Z", (third,))
        .await?;
    ...
    let empty_iterator: ClassInstanceRef<Object> = jvm
        .invoke_virtual(
            &empty_hash_set,
            &empty_hash_set.class_definition().name(),
            "iterator",
            "()Ljava/util/Iterator;",
            (),
        )
