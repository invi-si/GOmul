...
async fn test_data_input_stream() -> Result<()> {
    ...
    let available: i32 = jvm
        .invoke_virtual(&data_input_stream, &data_input_stream.class_definition().name(), "available", "()I", ())
        .await?;
    assert_eq!(available, data_len as i32);

    let byte: i8 = jvm
        .invoke_virtual(&data_input_stream, &data_input_stream.class_definition().name(), "readByte", "()B", ())
        .await?;
    assert_eq!(byte, 0x01);

    let short: i16 = jvm
        .invoke_virtual(&data_input_stream, &data_input_stream.class_definition().name(), "readShort", "()S", ())
        .await?;
    assert_eq!(short, 0x0203);

    let int: i32 = jvm
        .invoke_virtual(&data_input_stream, &data_input_stream.class_definition().name(), "readInt", "()I", ())
        .await?;
    assert_eq!(int, 0x04050607);

    let long: i64 = jvm
        .invoke_virtual(&data_input_stream, &data_input_stream.class_definition().name(), "readLong", "()J", ())
        .await?;
    assert_eq!(long, 0x08090a0b0c0d0e0f);

    let float: f32 = jvm
        .invoke_virtual(&data_input_stream, &data_input_stream.class_definition().name(), "readFloat", "()F", ())
        .await?;
    assert_eq!(float, f32::from_be_bytes([0x10, 0x11, 0x12, 0x13]));

    let double: f64 = jvm
        .invoke_virtual(&data_input_stream, &data_input_stream.class_definition().name(), "readDouble", "()D", ())
        .await?;
    assert_eq!(double, f64::from_be_bytes([0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b]));

    let utf: ClassInstanceRef<String> = jvm
        .invoke_virtual(
            &data_input_stream,
            &data_input_stream.class_definition().name(),
            "readUTF",
            "()Ljava/lang/String;",
            (),
        )
...
async fn test_skip_past_eof() -> Result<()> {
    ...
    let skipped: i64 = jvm.invoke_virtual(&dis, &dis.class_definition().name(), "skip", "(J)J", (10i64,)).await?;
    assert_eq!(skipped, 5);

    let read: i32 = jvm.invoke_virtual(&dis, &dis.class_definition().name(), "read", "()I", ()).await?;
    assert_eq!(read, -1);
