    ...
    async fn read(
        ...
        while write_buf_size < length {
            ...
            if !end_of_input && charset == "UTF-8" && decode_length > 0 {
                ...
                decode_length = complete_euc_kr_prefix(&read_buf_data);
            }
...
// Scan lead/trail pairs: a high final byte can be a complete pair's trail.
// Only an unmatched lead must wait for the next InputStream read.
fn complete_euc_kr_prefix(bytes: &[u8]) -> usize {
    let mut cursor = 0;
    while cursor < bytes.len() {
        if (0x81..=0xfe).contains(&bytes[cursor]) {
            if cursor + 1 == bytes.len() {
                return cursor;
            }
            let trail = bytes[cursor + 1];
            if (0x41..=0x5a).contains(&trail) || (0x61..=0x7a).contains(&trail) || (0x81..=0xfe).contains(&trail) {
                cursor += 2;
                continue;
            }
        }
...
#[cfg(test)]
mod korean_boundary_tests {
    use super::complete_euc_kr_prefix;
    use alloc::{string::String, vec::Vec};
    #[test]
    fn chunked_korean_matches_whole_input() {
        let text = "튜토리얼을 진행하겠습니까?\r\n1.예  2.아니오\r\n";
        ...
        for chunk_size in 1..=bytes.len() {
            ...
            for chunk in bytes.chunks(chunk_size) {
                ...
                let complete = complete_euc_kr_prefix(&pending);
                result.push_str(&encoding_rs::EUC_KR.decode(&pending[..complete]).0);
                pending.drain(..complete);
            }
            result.push_str(&encoding_rs::EUC_KR.decode(&pending).0);
            assert_eq!(result, text, "chunk size {chunk_size}");
        }
        assert_eq!(complete_euc_kr_prefix(&[0xc6, 0xa9]), 2);
        assert_eq!(complete_euc_kr_prefix(&[0xc6]), 0);
        assert_eq!(complete_euc_kr_prefix(&[0xc6, 0x20]), 2);
    }
}
