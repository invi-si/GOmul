...
macro_rules! profile_span {
    ($cpu:ident, $stage:ident, $expression:expr) => {{
        let token = $cpu.profiling.enter(crate::profiling::Stage::$stage);
        let result = $expression;
        $cpu.profiling.leave(token);
        result
    }};
}
...
#[cfg(feature = "profiling")]
macro_rules! profile_reg_write {
    ($cpu:ident, $register:expr, $expression:expr) => {{
        let token = $cpu.profiling.register_write($register);
        let result = $expression;
        $cpu.profiling.leave(token);
        result
    }};
...
#[derive(Copy, Clone)]
#[cfg_attr(not(feature = "profiling"), derive(PartialEq, Eq))]
...
// Observations are not architectural CPU state.
#[cfg(feature = "profiling")]
...
impl Cpu {
    ...
    pub fn new() -> Cpu {
        let mut cpu = Cpu {
            ...
            #[cfg(feature = "profiling")]
            profiling: profiling::Profiler::default(),
            #[cfg(feature = "advanced_disasm")]
            cs: Some(
                Capstone::new()
                    .arm()
    ...
    /// Configure host-side observations, resetting previous counters. The clock
    /// must be monotonic nanoseconds and must not mutate guest state or time.
    #[cfg(feature = "profiling")]
    pub fn set_profiling(
        &mut self,
        mode: profiling::ProfileMode,
        mean_interval: u32,
        seed: u32,
        clock: profiling::HostClock,
    ) {
        self.profiling.configure(mode, mean_interval, seed, clock);
    }

    /// Clear observations while preserving the selected mode, clock and seed.
    #[cfg(feature = "profiling")]
    pub fn reset_profiling(&mut self) {
        self.profiling.reset();
    }

    /// Copy counters without reading the clock or altering execution.
    #[cfg(feature = "profiling")]
    pub fn profiling_snapshot(&self) -> profiling::ProfileSnapshot {
    ...
    pub fn step(&mut self, mem: &mut impl Memory) -> bool {
        #[cfg(feature = "profiling")]
        self.profiling.begin_instruction();
        ...
        #[cfg(feature = "profiling")]
        self.profiling.end_execute();
        profile_span!(self, ExceptionCheck, {
            if !decode_ok {
                profile_span!(self, Exception, self.exception(Exception::Undefined))
            }
        });
        #[cfg(feature = "profiling")]
        self.profiling.end_instruction(decode_ok);
    ...
    pub fn exception(&mut self, exc: Exception) {
        ...
        let cpsr = profile_span!(self, FlagsCpsr, self.reg.get(0, reg::CPSR));
        ...
        profile_span!(self, FlagsCpsr, self.reg.set(new_bank, reg::SPSR, cpsr));

        profile_span!(self, BranchPc, self.reg.set(0, reg::PC, exc.address()));
        ...
        profile_span!(self, FlagsCpsr, self.reg.set(0, reg::CPSR, new_cpsr));
    }
    ...
    // Let fixed mode/register callers fold the bank lookup (especially PC/CPSR).
    #[inline]
...
#[cfg(all(test, not(feature = "profiling"), not(feature = "advanced_disasm")))]
mod profiling_disabled_tests {
    #[test]
    fn cpu_keeps_the_upstream_register_only_layout() {
        assert_eq!(
            std::mem::size_of::<super::Cpu>(),
            std::mem::size_of::<super::reg::RegFile>()
        );
        assert_eq!(
            std::mem::align_of::<super::Cpu>(),
            std::mem::align_of::<super::reg::RegFile>()
        );
    }
}
