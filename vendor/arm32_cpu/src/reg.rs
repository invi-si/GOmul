...
mod mapping_tests {
    ...
    fn current_reads_match_every_bank_and_register() {
        for (bank, map) in REG_MAP.iter().enumerate() {
            ...
            regs.bank = bank;
            for (i, value) in regs.reg.iter_mut().enumerate() {
                *value = 0x12340000 | i as u32;
            }
            for (reg, slot) in map.iter().enumerate() {
                assert_eq!(regs[reg as Reg], regs.reg[*slot]);
            }
        }
    }

    #[test]
    fn current_writes_match_every_bank_and_register() {
        for (bank, map) in REG_MAP.iter().enumerate() {
            for (reg, slot) in map.iter().enumerate() {
                let mut regs = RegFile::new_empty();
                regs.bank = bank;
                let mut expected = regs.reg;
                expected[*slot] = 0x12345678;
                regs[reg as Reg] = 0x12345678;
                assert_eq!(regs.reg, expected);
                assert_eq!(regs.bank, bank);
            }
        }
    }
}
